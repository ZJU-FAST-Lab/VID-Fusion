...
void Estimator::clearState()
{
    ...
    // Fz_avg_vector.clear(); //Fz_avg for average Fz in between Windows
    Fz_avg_vector.resize(WINDOW_SIZE + 1, 0.0);
...
{   
    ...
    {
        pre_integrations[frame_count] = new IntegrationBase{acc_0, gyr_0, Fz_0, Bas[frame_count], Bgs[frame_count], q_gt0}; 
    }
    if (frame_count != 0)
    {
        ...
        Vector3d control_acc_0 = Rs[j] * Vector3d(0.0, 0.0, Fz_0);
        Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - Bgs[j];
        Rs[j] *= Utility::deltaQ(un_gyr * dt).toRotationMatrix();
        Vector3d un_acc_1 = Rs[j] * (linear_acceleration - Bas[j]) - g;
        Vector3d control_acc_1 = Rs[j] * Vector3d(0.0, 0.0, Fz);
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);
        Vector3d control_acc = 0.5 * (control_acc_0 + control_acc_1);
        Ps[j] += dt * Vs[j] + 0.5 * dt * dt * un_acc; //propagation for init value of optimization 
        Vs[j] += dt * un_acc;
        Fexts_temp += (un_acc + g - control_acc) * dt;  // propagation for init value of optimization  instead of using the last one
        Fz_avg += 0.5 * (Fz_0 + Fz) * dt; //Fz_avg for average Fz in between Windows
    }
    acc_0 = linear_acceleration;
    gyr_0 = angular_velocity;
    Fz_0 = Fz;

    // TODO: Test switching to VIO from VID when estimator receives zero control input and vice versa
    // if (USE_VID)
    // {
    //     if (Fz <= 1e-3)
    //     {
    //         ROS_WARN("Receiving ZERO control input, switching OFF VID, switching ON VIO");
    //         use_vid = false;
    //     }
    //     else
    //     {
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Fz_0, Bas[frame_count], Bgs[frame_count], q_gt0};
    ...
    if (solver_flag == INITIAL)
    {
        ...
        {
            frame_count++;
            Fexts_temp = Vector3d(0, 0, 0);
            Fz_avg = 0; //Fz_avg for average Fz in between Windows
            sum_dt_preintegration_force = 0;
        }
...
bool Estimator::visualInitialAlign()
{
    ...
    ROS_INFO_STREAM("my R0  " << Utility::R2ypr(Rs[0]).transpose());  //ROS_DEBUG_STREAM
...
void Estimator::solveOdometry()
{
    ...
    if (solver_flag == NON_LINEAR)
    {
        ...
        // cout << "Fexts[" << frame_count << "]:  "  << Fexts[frame_count] << endl;
        optimization();
        // cout << "Fexts[" << frame_count << "]:  "  << Fexts[frame_count] << endl;
        // cout << "otimaization" << endl;
        // time_t finish_t = clock();
        // cout<<"optimization cost " << (double )(finish_t - begin_t )/CLOCKS_PER_SEC <<" s"<<endl;
...
void Estimator::double2vector()
{
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        ...
        //20210117 Change Fexts init
        // if (i == WINDOW_SIZE)//NOTE
...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if(use_vid)
        {
            ...
            problem.AddResidualBlock(model_factor, NULL, para_Position[i], para_Attitude[i], para_Speed[i], para_Fext[i], para_Bias[i], para_Position[j], para_Speed[j], para_Fext[j],  para_Bias[j]);  
            // std::cout << "para_Bias[i]: " << para_Bias[i][0] << " " << para_Bias[i][1] << " " << para_Bias[i][2] << std::endl;
            // std::cout << "para_Fext[i]: " << para_Fext[i][0] << " " << para_Fext[i][1] << " " << para_Fext[i][2] << std::endl;
            // std::cout << "----------------------------------------------" << std::endl;
        }         
    }
    int f_m_cnt = 0;
    ...
    // cout << "para_Fext[" << frame_count << "]:  "  << para_Fext[frame_count][0] << " " << para_Fext[frame_count][1] << " " << para_Fext[frame_count][2] << " " << endl;
            ...
            if(use_vid)
            {
                if (pre_integrations[1]->sum_dt < 10.0)
                {
                    ...
                    ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(model_factor, NULL,
                                                                               vector<double *>{para_Position[0], para_Attitude[0], para_Speed[0], para_Fext[0], para_Bias[0], para_Position[1], para_Speed[1], para_Fext[1], para_Bias[1]},
...
void Estimator::slideWindow()
{
    ...
    if (marginalization_flag == MARGIN_OLD) 
    {
        ...
        if (frame_count == WINDOW_SIZE)
        {
            ...
            Fexts[WINDOW_SIZE] = Fexts[WINDOW_SIZE - 1];// Change Fexts optimized results after change init of it

            // cout << "Fexts[WINDOW_SIZE]: " << Fexts[WINDOW_SIZE] << endl;
            Fexts_temp = Vector3d(0, 0, 0);
            Fz_avg_vector.at(WINDOW_SIZE) = Fz_avg / sum_dt_preintegration_force; //Fz_avg for average Fz between Windows
            Fz_avg = 0; //Fz_avg for average Fz in between Windows
            sum_dt_preintegration_force = 0;
            // std::cout << "slidingold..." << std::endl;

            delete pre_integrations[WINDOW_SIZE];
            pre_integrations[WINDOW_SIZE] = new IntegrationBase{acc_0, gyr_0, Fz_0, Bas[WINDOW_SIZE], Bgs[WINDOW_SIZE], q_gt0};
